...
function initializeCharts() {
    chartsConfig.forEach(cfg =>{
     ...
     createChart(cfg.id, cfg.type, cfg.data, cfg.options)
    });
}
...
async function loadStats() {
  ...
  try {
    const res = await fetch("https://vbnnzmhopcjlkvtuubcj.supabase.co/functions/v1/get-stats", {
      method: "GET",
      headers: {
    ...
    document.getElementById("page-views").textContent = data.total_visits ?? 0; // أو إذا عندك حقل آخر للمشاهدات

    // بإمكانك إضافة المزيد إذا قمت بتطوير الـ API لإرجاع bounce_rate أو session_duration
